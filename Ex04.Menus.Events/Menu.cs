...
namespace Ex04.Menus.Events
{
    public abstract class Menu : MenuItem
    {
        ...
        public Menu(string i_Title, Menu i_PreviousMenu, string i_EscPrint) 
        ...
        private int getChoiseFromUser()
        {
            ...
            while (!validInput)
            {
                Console.WriteLine("\nChoose an option from the menu : (from the range {0} to {1})", 0, r_MenuItems.Count);
                validInput = int.TryParse(Console.ReadLine(), out userInput);
                validInput = validInput && userInput >= 0 && userInput <= r_MenuItems.Count;
                if (!validInput)
                {
                    Console.WriteLine("Invalid input, please choose a number: (from the range {0} to {1})", 0, r_MenuItems.Count);
                }
            }
